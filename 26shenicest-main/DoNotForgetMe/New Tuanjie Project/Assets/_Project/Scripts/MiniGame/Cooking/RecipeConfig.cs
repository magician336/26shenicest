...
namespace DoNotForgetMe.MiniGame.Cooking
{
    ...
    public class RecipeConfig : ScriptableObject
    {
        ...
        [Tooltip("此配方使用的 CookingView prefab 路径（Resources 下）。留空则使用默认 MiniGamePrefabs/CookingView")]
        [SerializeField] private string viewPrefabName = "MiniGamePrefabs/CookingView";
        ...
        public string ViewPrefabName => string.IsNullOrEmpty(viewPrefabName) ? "MiniGamePrefabs/CookingView" : viewPrefabName;
        ...
        /// <summary>判断食材是否为正确食材（非干扰项）。</summary>
        public bool IsCorrectIngredient(string itemId)
        {
