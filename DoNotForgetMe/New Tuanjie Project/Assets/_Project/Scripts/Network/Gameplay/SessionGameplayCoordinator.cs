...
namespace DoNotForgetMe.Network.Gameplay
{
    ...
    public class SessionGameplayCoordinator : MonoBehaviour
    {
        ...
        private void EnsureDependencies()
        {
            ...
            {
                var go = new GameObject("MemoryAlbumController");
        ...
        private void InitializeSessionState()
        {
                ...
                && !State.dialogue.IsActive)
            {
                ...
                && !State.dialogue.IsActive)
            {
                // 八卦完成后回到 LivingRoom：自动启动相册小游戏
                var albumConfigId = albumConfigs != null && albumConfigs.Length > 0
                    ? albumConfigs[0].MiniGameId : "album_family_portrait";
                StartAlbumMiniGame(albumConfigId);
            }
        }
        ...
        public void RestoreHostSave(GameProgressSave save)
        {
            ...
            if (save.albumState != null)
            {
                ...
                if (save.albumState.phase == GameplayPhase.MiniGame ||
                    save.albumState.phase == GameplayPhase.MiniGameInterrupted)
                {
        ...
        private void InterruptMiniGame()
        {
            ...
            if (IsAlbumActive)
            {
                ...
                State.album.phase = GameplayPhase.MiniGameInterrupted;
                PublishStableState();
                return;
            }
            ...
            if (IsBaguaActive)
            {
                ...
                var result = _baguaLogic.Interrupt(State.bagua);
                if (result == BaguaSessionLogic.PublishKind.Stable)
                {
                    State.phase = GameplayPhase.MiniGameInterrupted;
                    State.bagua.phase = GameplayPhase.MiniGameInterrupted;
                    PublishStableState();
                }
                return;
            }
            ...
            State.phase = GameplayPhase.MiniGameInterrupted;
            State.cooking.phase = GameplayPhase.MiniGameInterrupted;
            PublishStableState();
        }
        ...
        private void ResumeMiniGame()
        {
            ...
            {
                State.phase = GameplayPhase.MiniGame;
                State.album.phase = GameplayPhase.MiniGame;
                PublishStableState();
                return;
            }
            ...
            if (IsBaguaActive)
            {
                var result = _baguaLogic.Resume(State.bagua);
                if (result == BaguaSessionLogic.PublishKind.Stable)
                {
                    State.phase = GameplayPhase.MiniGame;
                    State.bagua.phase = GameplayPhase.MiniGame;
                    PublishStableState();
                }
                return;
            }
            ...
            State.phase = GameplayPhase.MiniGame;
            State.cooking.phase = GameplayPhase.MiniGame;
            PublishStableState();
        }
        ...
        private void RestartMiniGame()
        {
            ...
            if (IsBaguaActive)
            {
                var result = _baguaLogic.Restart(State.bagua);
                if (result == BaguaSessionLogic.PublishKind.Stable)
                {
                    State.phase = GameplayPhase.MiniGame;
                    State.bagua.phase = GameplayPhase.MiniGame;
            ...
            var recipeId = State.cooking.recipeId;
            State.cooking = new CookingGameState
            {
                phase = GameplayPhase.MiniGame,
                recipeId = recipeId,
        ...
        private void FinishMiniGame()
        {
            ...
            if (IsAlbumActive)
            {
                ...
                State.album.phase = GameplayPhase.GameEnded;
                PublishStableState();
                return;
            }
