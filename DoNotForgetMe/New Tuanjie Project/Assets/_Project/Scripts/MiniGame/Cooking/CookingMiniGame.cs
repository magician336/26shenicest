...
namespace DoNotForgetMe.MiniGame.Cooking
{
    ...
    public class CookingMiniGame : MiniGameBase
    {
        ...
        public override void StartGame()
        {
            ...
            // 加载并实例化 prefab
            var prefab = Resources.Load<GameObject>("MiniGamePrefabs/CookingView");
            if (prefab == null)
            {
                Debug.LogError("[CookingMiniGame] 未找到 MiniGamePrefabs/CookingView prefab。" +
        ...
        private void RenderMotherView(CookingGameState state)
        {
            ...
            else
            {
                ...
                if (v.MotherWaitingText != null)
                {
                    v.MotherWaitingText.text = "菜已经做好了。请等待女儿端调味。";
                    v.MotherWaitingText.gameObject.SetActive(true);
                }
        ...
        private void RenderDaughterView(CookingGameState state)
        {
            ...
            {
                v.InterruptButtonRoot.SetActive(!state.completed);
            }
