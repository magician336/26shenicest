...
namespace DoNotForgetMe.EditorTools
{
    ...
    public static class AlbumPrefabExporter
    {
        ...
        private static void BuildHierarchy(GameObject root, AlbumConfig config)
        {
            ...
            // --- 中断按钮 ---
            var interruptBtn = CreateUIObject("InterruptButtonRoot", rt);
        ...
        private static void AutoWireReferences(GameObject root, AlbumView view, AlbumConfig config)
        {
            ...
            so.FindProperty("_interruptButtonRoot").objectReferenceValue = FindGameObject(rt, "InterruptButtonRoot");
            so.FindProperty("_interruptButton").objectReferenceValue = FindComponent<Button>(rt, "InterruptButtonRoot");
            so.FindProperty("_interruptButtonLabel").objectReferenceValue = FindComponent<Text>(rt, "InterruptButtonLabel");
