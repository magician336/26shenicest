...
public static class SceneSetupBase
{
    ...
    /// <summary>创建地面，宽度应匹配背景图横向长度。</summary>
    public static void CreateGround(float width = 20f)
    {
        CreatePlatform("Ground", new Vector3(0, -3, 0), new Vector3(width, 1, 1));
    }
    ...
    public static GameObject CreatePlayer(InputSettings inputSettings, PlayerSettings playerSettings, int interactableMask)
    {
        var playerObj = new GameObject("Player");
        playerObj.layer = PlayerLayer;
        playerObj.transform.position = new Vector3(0, PLAYER_Y, 0);
        ...
        DoNotForgetMe.MiniGame.Bagua.BaguaStoryConfig baguaConfig, AlbumConfig albumConfig)
    {
        ...
        // 对白配置
        var dlg1 = AssetDatabase.LoadAssetAtPath<DoNotForgetMe.Dialogue.DialogueSequence>("Assets/_Project/Dialogue/DLG_EnterMemory.asset");
        var dlg2 = AssetDatabase.LoadAssetAtPath<DoNotForgetMe.Dialogue.DialogueSequence>("Assets/_Project/Dialogue/DLG_Game1ToGame2.asset");
        var dlg3 = AssetDatabase.LoadAssetAtPath<DoNotForgetMe.Dialogue.DialogueSequence>("Assets/_Project/Dialogue/DLG_Game2ToGame3.asset");
        var dlg4 = AssetDatabase.LoadAssetAtPath<DoNotForgetMe.Dialogue.DialogueSequence>("Assets/_Project/Dialogue/DLG_Ending.asset");
        var dialogues = new[] { dlg1, dlg2, dlg3, dlg4 };
    ...
    /// <summary>创建电影质感字幕条（底部黑色留白区域 + TMP 文字），宽度匹配背景图。</summary>
    public static void CreateSubtitleBar(float bgWorldWidth = 0f)
    {
        var aspect = (float)Screen.width / Screen.height;
        if (aspect <= 0) aspect = 16f / 9f;
        float viewWidth = CAMERA_ORTHO_SIZE * 2f * aspect;
        // 字幕条宽度取背景宽度和视野宽度中较大者，确保完全覆盖底部
        ...
        // TMP 文字（独立于字幕条缩放，避免被拉伸）
        var textObj = new GameObject("SubtitleText");
        textObj.transform.position = new Vector3(0, SUBTITLE_BAR_Y, -1);
        ...
        // 挂载字幕控制器
        barObj.AddComponent<CinematicSubtitle>();
    }
    ...
    public static void WireArtAssets()
    {
        ...
        if (bagua != null)
        {
            ...
            var itemSprites = new (string itemId, string path)[] {
                ("key",           ITEM_KEY_PATH),
                ("tin_candy_box", ITEM_TIN_CANDY_PATH),
