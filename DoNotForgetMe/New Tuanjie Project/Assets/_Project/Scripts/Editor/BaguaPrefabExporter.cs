...
namespace DoNotForgetMe.EditorTools
{
    ...
    public static class BaguaPrefabExporter
    {
        ...
        private static void BuildHierarchy(GameObject root, BaguaStoryConfig config)
        {
            ...
            // 中断按钮
            var interruptBtn = CreateUIObject("InterruptButtonRoot", hostPanel.transform);
            var ibRect = interruptBtn.GetComponent<RectTransform>();
            ibRect.anchorMin = ibRect.anchorMax = new Vector2(0.5f, 0.5f);
            ibRect.anchoredPosition = new Vector2(730, -460);
            ibRect.sizeDelta = new Vector2(250, 110);
            var ibImg = interruptBtn.AddComponent<Image>();
            ibImg.color = new Color(0.42f, 0.32f, 0.2f);
            ...
            var ibLabel = CreateTextObject("InterruptButtonLabel", interruptBtn.transform,
                "暂时离开", 28, Color.white, Vector2.zero);
        ...
        private static void AutoWireReferences(GameObject root, BaguaView view, BaguaStoryConfig config)
        {
            ...
            so.FindProperty("_interruptButtonRoot").objectReferenceValue = FindGameObject(hostPanel, "InterruptButtonRoot");
            so.FindProperty("_interruptButton").objectReferenceValue = FindComponent<Button>(hostPanel, "InterruptButtonRoot");
            so.FindProperty("_interruptButtonLabel").objectReferenceValue = FindComponent<Text>(hostPanel, "InterruptButtonLabel");
